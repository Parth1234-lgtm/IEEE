...
(function () {
  ...
  function injectFeedbackBar() {
    ...
    document.getElementById("adaptFbClose").addEventListener("click", () => {
      const btns = document.getElementById("adaptFbButtons");
      minimized = !minimized;
      btns.style.display = minimized ? "none" : "flex";
  ...
  chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
    ...
    if (msg.type === "APPLY_STATE_NOW" || msg.type === "APPLY_STATE") {
      ...
      applyStateAdaptations(state);
      updateFloatingBar();
      console.log(`[content] >>> ${msg.type} complete — "${state}" applied, applied="${applied}".`);
      sendResponse({ ok: true, applied: state });
      return true;
    }
